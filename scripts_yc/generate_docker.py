...
maxlr = 5e-3
n_workers = 8
# checkpoint_root = '/scratch/ychen855/leTetCNN/checkpoints'
checkpoint_root = '/data/hohokam/Yanxi/Code/leTetCNN/checkpoints'

host_data = root
vm_data = '/data'
# host_code = '/scratch/ychen855/leTetCNN'
host_code = '/data/hohokam/Yanxi/Code/leTetCNN'
vm_code = '/leTetCNN'
# image = '/scratch/ychen855/tetcnn.sif'
            ...
            for network in networks:
                for pair in exp_pairs:
                    ...
                    for file_pair in pos_neg:
                        ...
                        if landmark:
                            out_lines.append('#SBATCH -o /data/hohokam/Yanxi/Code/leTetCNN/scripts_yc/job_logs/' + str(pos_name) + '_' + str(neg_name) + '_' + str(n_lmk) + '_' + str(k) + '_' + network + '_' + str(fold) + '.out')
                            out_lines.append('#SBATCH -e /data/hohokam/Yanxi/Code/leTetCNN/scripts_yc/job_logs/' + str(pos_name) + '_' + str(neg_name) + '_' + str(n_lmk) + '_' + str(k) + '_' + network + '_' + str(fold) + '.err')
                        else:
                            out_lines.append('#SBATCH -o /data/hohokam/Yanxi/Code/leTetCNN/scripts_yc/job_logs/' + str(pos_name) + '_' + str(neg_name) + '_tetcnn' + '_' + str(fold) + '.out')
                            out_lines.append('#SBATCH -e /data/hohokam/Yanxi/Code/leTetCNN/scripts_yc/job_logs/' + str(pos_name) + '_' + str(neg_name) + '_tetcnn' + '_' + str(fold) + '.err')

                        # if landmark:
                        #     out_lines.append('#SBATCH -o /scratch/ychen855/leTetCNN/scripts/job_logs/' + str(pos_name) + '_' + str(neg_name) + '_' + str(n_lmk) + '_' + str(k) + '_' + network + '_' + str(fold) + '.out')
                        #     out_lines.append('#SBATCH -e /scratch/ychen855/leTetCNN/scripts/job_logs/' + str(pos_name) + '_' + str(neg_name) + '_' + str(n_lmk) + '_' + str(k) + '_' + network + '_' + str(fold) + '.err')
                        # else:
                        #     out_lines.append('#SBATCH -o /scratch/ychen855/leTetCNN/scripts/job_logs/' + str(pos_name) + '_' + str(neg_name) + '_tetcnn' + '_' + str(fold) + '.out')
                        #     out_lines.append('#SBATCH -e /scratch/ychen855/leTetCNN/scripts/job_logs/' + str(pos_name) + '_' + str(neg_name) + '_tetcnn' + '_' + str(fold) + '.err')
                        ...
                        if landmark:
                            out_lines.append('python /scratch/ychen855/leTetCNN/src/tetCNN_lm.py --pos ${POS} --neg ${NEG} --n_workers ${N_WORKERS} --maxlr ${MAXLR} --val_freq ${VAL_FREQ} --save_freq ${SAVE_FREQ} --checkpoint_dir ${CHECKPOINT_DIR} --landmark --n_lmk ${LMK} --bnn ${BNN} --n_epoch ${N_EPOCH} --network ${NETWORK} --lr ${LR} --wd ${WD} --fold ${FOLD} --n_fold ${N_FOLD} --load')
                        else:
                            out_lines.append('apptainer exec --bind ${HOST_DATA}:${VM_DATA},${HOST_CODE}:${VM_CODE} ${IMAGE} python3 ${VM_CODE}/src/tetCNN_lm.py --pos ${POS} --neg ${NEG} --n_workers ${N_WORKERS} --maxlr ${MAXLR} --val_freq ${VAL_FREQ} --save_freq ${SAVE_FREQ} --checkpoint_dir ${CHECKPOINT_DIR} --n_epoch ${N_EPOCH} --lr ${LR} --wd ${WD} --fold ${FOLD} --n_fold ${N_FOLD} --load')
