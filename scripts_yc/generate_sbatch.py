            ...
            for network in networks:
                for pair in exp_pairs:
                    ...
                    for file_pair in pos_neg:
                        ...
                        if landmark:
                            out_lines.append('#SBATCH -o /scratch/ychen855/leTetCNN/scripts/job_logs/' + str(pos_name) + '_' + str(neg_name) + '_' + str(n_lmk) + '_' + str(k) + '_' + network + '_' + str(cv) + '.out')
                            out_lines.append('#SBATCH -e /scratch/ychen855/leTetCNN/scripts/job_logs/' + str(pos_name) + '_' + str(neg_name) + '_' + str(n_lmk) + '_' + str(k) + '_' + network + '_' + str(cv) + '.err')
                        else:
                            out_lines.append('#SBATCH -o /scratch/ychen855/leTetCNN/scripts/job_logs/' + str(pos_name) + '_' + str(neg_name) + '_tetcnn' + '_' + str(cv) + '.out')
                            out_lines.append('#SBATCH -e /scratch/ychen855/leTetCNN/scripts/job_logs/' + str(pos_name) + '_' + str(neg_name) + '_tetcnn' + '_' + str(cv) + '.err')
                        ...
                        if landmark:
                            out_lines.append('python /scratch/ychen855/leTetCNN/src/tetCNN_lm.py --pos ${POS} --neg ${NEG} --n_workers ${N_WORKERS} --maxlr ${MAXLR} --val_freq ${VAL_FREQ} --save_freq ${SAVE_FREQ} --checkpoint_dir ${CHECKPOINT_DIR} --landmark --n_lmk ${LMK} --k ${K} --n_epoch ${N_EPOCH} --network ${NETWORK} --lr ${LR} --wd ${WD} --cv ${CV} --load')
                        else:
                            out_lines.append('python /scratch/ychen855/leTetCNN/src/tetCNN_lm.py --pos ${POS} --neg ${NEG} --n_workers ${N_WORKERS} --maxlr ${MAXLR} --val_freq ${VAL_FREQ} --save_freq ${SAVE_FREQ} --checkpoint_dir ${CHECKPOINT_DIR} --n_epoch ${N_EPOCH} --lr ${LR} --wd ${WD} --cv ${CV} --load')
