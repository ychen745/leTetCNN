...
import util_lm
from network import superglue

class SuperNode(tnn.Module):
    def __init__(self, feat_dim, n_layer=2, network='mlp'):
        super().__init__()
        kenc_layers = [feat_dim, feat_dim * 2, feat_dim * 2, feat_dim]
        # for i in range(n_layer):
        #     kenc_layers.append(feat_dim * (2 ** i))
        self.network = network
        self.kenc = superglue.KeypointEncoder(feat_dim, kenc_layers, use_layernorm=False)
        self.gnn = superglue.AttentionalGNN(feat_dim, ['self', 'cross'] * 6, use_layernorm=False)
        self.distnet = ChebConv2(feat_dim, feat_dim, 1)

    def forward(self, data):
        kpts = data.x[data.lmk == 1]
        d_map = torch.zeros(data.x.shape)

        ### MLP projector ###
        if self.network == 'mlp':
            kpts_code = self.kenc(kpts.unsqueeze(0)).squeeze(0).transpose(0, 1)
        ### attention layers ###
        elif self.network == 'gnn':
            kpts_code = kpts
            kpts_code, _ = self.gnn(kpts_code.unsqueeze(0).transpose(1, 2), kpts_code.unsqueeze(0).transpose(1, 2))
            kpts_code = kpts_code.squeeze(0).transpose(0, 1)
        elif self.network == 'kaf':
            kpts_code = self.kenc(kpts.unsqueeze(0)).squeeze(0).transpose(0, 1)
            kpts_code, _ = self.gnn(kpts_code.unsqueeze(0).transpose(1, 2), kpts_code.unsqueeze(0).transpose(1, 2))
            kpts_code = kpts_code.squeeze(0).transpose(0, 1)
        else:
        ...
        d_map[data.lmk == 1] = kpts_code
        d_map = F.relu(self.distnet(d_map, data.edge_index, data.LBO_index,data.LBO_weight,batch = data.batch,lambda_max = data.lmax))
        d_map = F.relu(self.distnet(d_map, data.edge_index, data.LBO_index,data.LBO_weight,batch = data.batch,lambda_max = data.lmax))
...
def avg_pool(cluster, data, transform=None):
    ...
    lmk = None if data.lmk is None else update_lmk(cluster, data.lmk)
...
#     data = Batch(batch=batch, x=x, edge_index=index, edge_attr=attr, pos=pos,LBO_index = LBO_index, LBO_weight=LBO_weight,mass = mass,lmax = lmax)
    data = Batch(batch=batch, x=x, edge_index=index, edge_attr=attr, pos=pos,LBO_index = LBO_index, LBO_weight=LBO_weight,lmax = lmax, mass = mass, lmk = lmk)
...
class Net(torch.nn.Module):
    def __init__(self, keypoint = False, network='mlp'):
        ...
        self.supernode1 = SuperNode(16, 2, network=self.network)
        self.supernode2 = SuperNode(32, 2, network=self.network)
        self.supernode3 = SuperNode(64, 3, network=self.network)
        self.supernode4 = SuperNode(128, 3, network=self.network)
    ...
    def forward(self, data, training):
        ...
        if self.keypoint:
            data.x = self.supernode1(data)
            # data.x = F.relu(self.conv12(data.x, data.edge_index, data.LBO_index,data.LBO_weight,batch = data.batch,lambda_max = data.lmax))
            # data.x = F.relu(self.conv12(data.x, data.edge_index, data.LBO_index,data.LBO_weight,batch = data.batch,lambda_max = data.lmax))
        ...
        if self.keypoint:
            data.x = self.supernode2(data)
            # data.x = F.relu(self.conv23(data.x, data.edge_index, data.LBO_index,data.LBO_weight,batch = data.batch,lambda_max = data.lmax))
            # data.x = F.relu(self.conv23(data.x, data.edge_index, data.LBO_index,data.LBO_weight,batch = data.batch,lambda_max = data.lmax))
        ...
        if self.keypoint:
            data.x = self.supernode3(data)
            # data.x = F.relu(self.conv34(data.x, data.edge_index, data.LBO_index,data.LBO_weight,batch = data.batch,lambda_max = data.lmax))
            # data.x = F.relu(self.conv34(data.x, data.edge_index, data.LBO_index,data.LBO_weight,batch = data.batch,lambda_max = data.lmax))
