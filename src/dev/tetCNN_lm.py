...
def load_data(pos_folders, neg_folders, n_lmk, BNN, load, cv):
    ...
    if load:   # if run python tetCNN_LBO.py --load it will go through loading all
        ...
        for i in range(len(pos_folders)):
            ...
            pos_files_tet = util_lm.list_files(pos_folder)
            for sample in pos_files_tet:
                pos_tet_list.append(tet_mesh.load_mesh(sample))

            pos_files_LBO = util_lm.read_cot(pos_folder)
            for sample in pos_files_LBO:
                pos_LBO_list.append(util_lm.load_LBO(sample, 'L').astype(np.float32))
            
            pos_files_mass = util_lm.read_mass(pos_folder)
            for sample in pos_files_mass:
                pos_mass_list.append(util_lm.load_LBO(sample, 'm').astype(np.float32))

            pos_files_lmk = util_lm.read_lmk(pos_folder, n_lmk, BNN)
            for sample in pos_files_lmk:
                pos_lmk_list.append(util_lm.load_LBO(sample, 'lmk').astype(np.float32))

            ############### CTL or any other groups
        for i in range(len(neg_folders)):
            neg_folder = neg_folders[i]
            
            neg_files_tet = util_lm.list_files(neg_folder)
            for sample in neg_files_tet:
                neg_tet_list.append(tet_mesh.load_mesh(sample))

            neg_files_LBO = util_lm.read_cot(neg_folder)
            for sample in neg_files_LBO:
                neg_LBO_list.append(util_lm.load_LBO(sample, 'L').astype(np.float32))
            
            neg_files_mass = util_lm.read_mass(neg_folder)
            for sample in neg_files_mass:
                neg_mass_list.append(util_lm.load_LBO(sample, 'm').astype(np.float32))

            neg_files_lmk = util_lm.read_lmk(neg_folder, n_lmk, BNN)
            for sample in neg_files_lmk:
                neg_lmk_list.append(util_lm.load_LBO(sample, 'lmk').astype(np.float32))

        pos_list_tet_data = util_lm.tet_data_LBO(pos_tet_list, pos_LBO_list, pos_mass_list, pos_lmk_list, True)
        neg_list_tet_data = util_lm.tet_data_LBO(neg_tet_list, neg_LBO_list, neg_mass_list, neg_lmk_list, False)
        ###combinig two together
        list_tet_data = pos_list_tet_data + neg_list_tet_data #+ MCI_list_tet_data
        # util_lm.save_data(list_tet_data, 'tetmesh_198_left.dat')

    else:
        print('here')
        list_tet_data = util_lm.load_data('tetmesh_198_left.dat', n_lmk, BNN)
    ...
    if cv == 9:
        train_dataset = list_tet_data[:len(list_tet_data)*cv//10]
    else:
        train_dataset = list_tet_data[:len(list_tet_data)*cv//10] + list_tet_data[len(list_tet_data)*(cv+1)//10:]
    test_dataset = list_tet_data[len(list_tet_data)*cv//10:len(list_tet_data)*(cv+1)//10]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_lmk', type=int, dest='n_lmk')
    parser.add_argument('--k', type=int, dest='k')
    parser.add_argument('--epoch', type=int, dest='n_epoch')
    parser.add_argument('--keypoint', action='store_true')
    parser.add_argument('--network', type=str, dest='network')
    parser.add_argument('--lr', type=float, dest='lr')
    parser.add_argument('--wd', type=float, dest='wd')
    ...
    model = torch.load('/scratch/ychen855/tetCNN/tetcnn.pth').to(device)
    ...
    for epoch in range(1, args.n_epoch+1):
        ...
        # if epoch % 20 == 0:
        #     torch.save(model, os.path.join('/scratch/ychen855/tetCNN/checkpoints', args.network + '_' + str(args.cv) + '_' + str(epoch) + '.pth'))
