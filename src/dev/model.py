...
def avg_pool_mass(cluster, x, batch, size: Optional[int] = None):
    r"""Average pools node features according to the clustering defined in
    :attr:`cluster`.
    See :meth:`torch_geometric.nn.pool.max_pool_x` for more details.
    ...
    if size is not None:
        batch_size = int(batch.max().item()) + 1
        return _avg_pool_mass(cluster, x, batch_size * size), None

    cluster, perm = consecutive_cluster(cluster)
    x = _avg_pool_mass(cluster, x)
    batch = pool_batch(perm, batch)
...
#         print(type(lambda_max))
    ...
    def forward(self, x, edge_index,LBO_index,LBO_weight, edge_weight: OptTensor = None,
        ...
        Tx_0 = x
        Tx_1 = x  # Dummy.
        out = torch.matmul(Tx_0.double(), self.weight[0].double())

        # propagate_type: (x: Tensor, norm: Tensor)
        if self.weight.size(0) > 1:
            Tx_1 = self.propagate(edge_index, x=x, norm=norm, size=None)
            out = out + torch.matmul(Tx_1.double(), self.weight[1].double())

        for k in range(2, self.weight.size(0)):
            Tx_2 = self.propagate(edge_index, x=Tx_1, norm=norm, size=None)
            Tx_2 = 2. * Tx_2 - Tx_0
            out = out + torch.matmul(Tx_2.double(), self.weight[k].double())
            Tx_0, Tx_1 = Tx_1, Tx_2
...
class VisulizeAttention(torch.nn.Module):
    def __init__(self):
        ...
        self.mid1 = torch.nn.Linear(64,32).double()
        self.tanh = torch.nn.Tanh()
        self.mid2 = torch.nn.Linear(32,1).double()
...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.fc1 = torch.nn.Linear(128,32).double()
        self.fc2 = torch.nn.Linear(32,2).double()
        # self.fc3 = torch.nn.Linear(32,2).double()
        self.bn1 = nn.BatchNorm(16).double()
        self.bn2 = nn.BatchNorm(32).double()
        self.bn3 = nn.BatchNorm(64).double()
        self.bn4 = nn.BatchNorm(128).double()
        self.bn5 = nn.BatchNorm(256).double()
        self.bn6 = nn.BatchNorm(32).double()
        self.weight = Parameter(torch.Tensor(64,64))
        glorot(self.weight)
        self.weight2 = Parameter(torch.Tensor(128,128))
        glorot(self.weight2)
        self.lin = Linear(64, 2).double()
    ...
    def forward(self, data, training):
        ...
        # data.x = F.dropout(data.x, p = 0.25, training = self.training)
