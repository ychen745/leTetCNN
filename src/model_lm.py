...
class BatchNorm1(torch.nn.Module):
...
class Net(torch.nn.Module):
    ...
    def forward(self, data, training):
        ...
        if self.keypoint:
            data.x = self.supernode1(data.x, data.lmk)
        ...
        if self.keypoint:
            data.x = self.supernode2(data.x, data.lmk)
        ...
        if self.keypoint:
            data.x = self.supernode3(data.x, data.lmk)
